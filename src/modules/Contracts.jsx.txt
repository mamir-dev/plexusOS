...
const GlobalStyles = () => (
    <style>{`
        ...
        .con-search input { background: transparent; border: none; outline: none; }
...
const StatCard = ({ label, value, sub, icon, color = 'cyan' }) => {
    ...
    // Theme-based colors for text
    const valueColor = dark ? '#f1f5f9' : '#0f172a';
    const labelColor = dark ? 'rgba(148,163,184,0.60)' : 'rgba(71,85,105,0.70)';
    const subColor = dark ? 'rgba(148,163,184,0.45)' : 'rgba(71,85,105,0.55)';
    ...
    return (
        <div className={`con-glass con-stat-${color} con-enter rounded-2xl p-5 flex flex-col gap-4 cursor-default`}>
            ...
            <div>
                <div className="con-mono" style={{ fontSize: 26, fontWeight: 800, color: valueColor, lineHeight: 1.1, letterSpacing: '-0.02em' }}>{value}</div>
                <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: labelColor, marginTop: 3 }}>{label}</div>
                <div style={{ fontSize: 12, color: subColor, marginTop: 2 }}>{sub}</div>
            </div>
        </div>
    );
};
...
/* ─── SectionHeader ───────────────────────────────────────────────────── */
const SectionHeader = ({ title, right, dark }) => (
    <div className="con-section-title" style={{ marginBottom: 16, justifyContent: 'flex-start', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
...
export function ContractsModule() {
    ...
    const textSecondary = dark ? 'rgba(148,163,184,0.60)' : 'rgba(71,85,105,0.70)';
    const textTertiary = dark ? 'rgba(148,163,184,0.45)' : 'rgba(71,85,105,0.55)';
    const textMuted = dark ? 'rgba(148,163,184,0.50)' : 'rgba(71,85,105,0.60)';
    const borderColor = dark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.08)';
    const cardBg = dark ? 'rgba(255,255,255,0.03)' : 'rgba(255,255,255,0.5)';
    const inputColor = dark ? '#f1f5f9' : '#0f172a';
    const tableHeaderColor = dark ? 'rgba(148,163,184,0.50)' : 'rgba(71,85,105,0.70)';
    const tableTextColor = dark ? '#e2e8f0' : '#334155';
        ...
        <div className="con-module" style={{ display: 'flex', flexDirection: 'column', gap: 20, width: '100%' }}>
                ...
                <div key="repository" style={{ display: 'flex', flexDirection: 'column', gap: 16 }} className="con-enter">
                    ...
                    <div className="con-banner con-enter rounded-2xl" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '14px 20px', flexWrap: 'wrap' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                            ...
                            <div>
                                <div style={{ fontSize: 13, fontWeight: 700, color: '#0891b2', marginBottom: 2 }}>Digitization Target: 30+ contracts remain in filing cabinets</div>
                                <div style={{ fontSize: 12, color: 'rgba(8, 145, 178, 0.9)' }}>Target: 80% digitized by Day 45 of pilot. Upload outstanding provider agreements ASAP.</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="con-glass con-enter rounded-2xl" style={{ overflow: 'hidden' }}>
                        ...
                        <div style={{
                            ...
                            gap: 12, padding: '16px 20px', borderBottom: `1px solid ${borderColor}`,
                        }}>
                            <div className="con-search" style={{
                                ...
                                <input
                                    style={{ flex: 1, fontSize: 13, fontWeight: 500, fontFamily: 'Outfit,sans-serif', minWidth: 0, color: inputColor }}
                                ...
                                return (
                                    <div key={c.id} className="con-row" style={{
                                        ...
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ fontSize: 14, fontWeight: 700, color: textPrimary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: 8 }}>{c.name}</div>
                                            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8 }}>
                                                <Badge color={typeColor}>{c.type}</Badge>
                                                <span style={{
                                                    fontSize: 11, fontWeight: 500, color: textMuted,
                                                    paddingLeft: 8, borderLeft: `1px solid ${borderColor}`, whiteSpace: 'nowrap',
                                                }}>{c.sites}</span>
                                            </div>
                                        </div>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexShrink: 0 }}>
                                            <div style={{ textAlign: 'right' }}>
                                                <div className="con-mono" style={{ fontSize: 13, fontWeight: 700, color: textPrimary, letterSpacing: '-0.01em' }}>{c.value}</div>
                                                <div style={{ fontSize: 11, color: textTertiary, marginTop: 2 }}>Exp: {c.expiry}</div>
                                            </div>
                            ...
                            <div key={emp.provider} className="con-emp-card con-enter" style={{
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                        ...
                                        <span style={{ fontSize: 14, fontWeight: 700, color: textPrimary }}>{emp.provider}</span>
                                    </div>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(120px,1fr))', gap: 12 }}>
                                    {[
                                        ...
                                        <div key={f.label}>
                                            <div style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: textSecondary, marginBottom: 4 }}>{f.label}</div>
                                            <div className="con-mono" style={{ fontSize: 13, fontWeight: 700, color: tableTextColor, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.value}</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
            ...
            {tab === "lifecycle" && (
                <div key="lifecycle" className="con-glass con-enter rounded-2xl p-6" style={{ display: 'flex', flexDirection: 'column' }}>
                    <SectionHeader title="Contract Renewal Calendar" dark={dark} />
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                        {[
                            ...
                            const daysColor = c.daysLeft < 60 ? '#fbbf24' : textPrimary;
                            ...
                            return (
                                <div key={c.name} className="con-lifecycle-card con-enter" style={{
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 14, flex: 1, minWidth: 0, overflow: 'hidden' }}>
                                        ...
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ fontSize: 13, fontWeight: 700, color: textPrimary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.name}</div>
                                            <div style={{ fontSize: 11, color: textMuted, marginTop: 3 }}>Expires: {c.expiry}</div>
                                        </div>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 18, flexShrink: 0 }}>
                                        <div style={{ textAlign: 'right' }}>
                                            ...
                                            <div style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', color: textTertiary, marginTop: 1 }}>remaining</div>
                                        </div>
            ...
            {tab === "payor" && (
                <div key="payor" className="con-glass con-enter rounded-2xl p-6" style={{ marginBottom: 24 }}>
                    <SectionHeader title="Payor Rate Comparison — Select CPT Codes" dark={dark} />
                            ...
                            <tbody>
                                {[
                                    ...
                                    <tr key={row.cpt} className="con-table-row" style={{ cursor: 'pointer' }}>
                                        ...
                                        <td style={{ padding: '14px 18px', fontSize: 13, fontWeight: 500, color: tableTextColor }}>{row.desc}</td>
                                        <td style={{ padding: '14px 18px', textAlign: 'right' }}>
                                            <span className="con-mono" style={{ fontSize: 13, fontWeight: 600, color: textMuted }}>{row.mc}</span>
                                        </td>
                                        <td style={{ padding: '14px 18px', textAlign: 'right' }}>
                                        ...
                                        <td style={{ padding: '14px 18px', textAlign: 'right' }}>
                                            <span className="con-mono" style={{ fontSize: 13, fontWeight: 600, color: textMuted }}>{row.ae}</span>
                                        </td>
                                        <td style={{ padding: '14px 18px', textAlign: 'right' }}>
