...
const GlobalStyles = ({ dark }) => (
    <style>{`
        ...
        /* Glass card base - dynamic based on theme */
        .fin-glass {
        ...
        .fin-glass:hover {
            box-shadow: ${dark 
                ? '0 16px 48px rgba(0,0,0,0.36), inset 0 1px 0 rgba(255,255,255,0.10)'
                : '0 16px 48px rgba(0,0,0,0.08), inset 0 1px 0 rgba(255,255,255,0.5)'};
            border-color: ${dark ? 'rgba(255,255,255,0.13)' : 'rgba(0,0,0,0.08)'};
        }
        ...
        /* Import banner */
        .fin-import-banner {
            background: ${dark ? 'rgba(59,130,246,0.09)' : 'rgba(59,130,246,0.03)'};
            border: 1px solid ${dark ? 'rgba(59,130,246,0.22)' : 'rgba(59,130,246,0.15)'};
            backdrop-filter: blur(12px);
            box-shadow: 0 4px 20px rgba(59,130,246,0.10), inset 0 1px 0 ${dark ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.5)'};
            transition: border-color 0.3s ease, box-shadow 0.3s ease;
        }
        .fin-import-banner:hover {
            border-color: rgba(59,130,246,0.38);
            box-shadow: 0 6px 28px rgba(59,130,246,0.18), inset 0 1px 0 ${dark ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.5)'};
        }
        ...
        .fin-alert-amber {
            ...
            background: ${dark ? 'rgba(245,158,11,0.08)' : 'rgba(245,158,11,0.03)'};
            backdrop-filter: blur(12px);
            box-shadow: 0 4px 20px rgba(245,158,11,0.10), inset 0 1px 0 ${dark ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.5)'};
        }
...
const StatCard = ({ label, value, sub, icon, color = 'blue', trend }) => {
    ...
    return (
        <div className={`fin-glass fin-stat-${color} fin-enter rounded-2xl p-5 flex flex-col gap-4 cursor-default`}>
            ...
            <div>
                <div className="fin-mono" style={{ fontSize: 26, fontWeight: 800, color: dark ? '#f1f5f9' : '#0f172a', lineHeight: 1.1, letterSpacing: '-0.02em' }}>{value}</div>
                <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: dark ? 'rgba(148,163,184,0.65)' : 'rgba(51,65,85,0.65)', marginTop: 3 }}>{label}</div>
                <div style={{ fontSize: 12, color: dark ? 'rgba(148,163,184,0.50)' : 'rgba(51,65,85,0.50)', marginTop: 2 }}>{sub}</div>
            </div>
        </div>
    );
};
...
const SectionHeader = ({ title, right }) => {
    ...
    return (
        <div className="fin-section-title" style={{ marginBottom: 16, justifyContent: 'space-between' }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: dark ? '#f1f5f9' : '#0f172a', letterSpacing: '-0.01em' }}>{title}</span>
            {right && <div>{right}</div>}
        </div>
    );
};
        ...
        <div className="fin-import-banner fin-enter rounded-2xl" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '14px 20px', flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <div style={{
                    ...
                    background: 'rgba(59,130,246,0.18)', color: '#3b82f6', flexShrink: 0,
                }}>
                    <FolderDown size={20} strokeWidth={1.5} />
                </div>
                <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#3b82f6', marginBottom: 2 }}>Data Import Engine Active</div>
                    <div style={{ fontSize: 12, color: dark ? 'rgba(148,163,184,0.60)' : 'rgba(51,65,85,0.60)' }}>Upload CSV, Excel, or PDF billing reports — inbox watching active.</div>
                </div>
            </div>
...
const AlertBanner = ({ icon, title, desc }) => {
    ...
    return (
        <div className="fin-alert-amber fin-enter rounded-2xl p-4" style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
            ...
            <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#f59e0b', marginBottom: 3 }}>{title}</div>
                <div style={{ fontSize: 12, color: dark ? 'rgba(148,163,184,0.65)' : 'rgba(51,65,85,0.65)', lineHeight: 1.5 }}>{desc}</div>
            </div>
        </div>
    );
};
...
export function FinancialModule() {
    ...
    <span style={{ fontSize: 14, fontWeight: 700, color: dark ? '#f1f5f9' : '#0f172a', letterSpacing: '-0.01em' }}>Cash Position Trend (8 Months)</span>
</div>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                                {["Aug","Sep","Oct","Nov","Dec","Jan","Feb","Mar"].map(m => (
                                    <span key={m} className="fin-mono" style={{ fontSize: 10, fontWeight: 500, color: dark ? 'rgba(148,163,184,0.45)' : 'rgba(51,65,85,0.45)', textAlign: 'center', flex: 1 }}>{m}</span>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="fin-glass fin-enter rounded-2xl p-6" style={{ display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div className="fin-section-title" style={{ marginBottom: 16, justifyContent: 'flex-start', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
    <span style={{ fontSize: 14, fontWeight: 700, color: dark ? '#f1f5f9' : '#0f172a', letterSpacing: '-0.01em' }}>Cash Flow Summary — Feb 2026</span>
</div>
                            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, justifyContent: 'center' }}>
                                {[
                                    { label: "Total Revenue",  value: fmtCurrency(445500), color: '#10b981' },
                                    { label: "Total Expenses", value: fmtCurrency(295000), color: '#ef4444' },
                                ].map(row => (
                                    <div key={row.label} className="fin-cf-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 0' }}>
                                        <span style={{ fontSize: 14, fontWeight: 500, color: dark ? 'rgba(148,163,184,0.75)' : 'rgba(51,65,85,0.75)' }}>{row.label}</span>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 20, marginTop: 4 }}>
                                    <span style={{ fontSize: 15, fontWeight: 800, color: dark ? '#f1f5f9' : '#0f172a' }}>Net Income</span>
                                    <span className="fin-mono" style={{
                                        fontSize: 22, fontWeight: 800, color: '#10b981', letterSpacing: '-0.02em',
                                        textShadow: dark ? '0 0 20px rgba(16,185,129,0.35)' : 'none',
                                    ...
                                    {mockData.revenueByPayor.map(p => (
                                        <div key={p.name} className="fin-legend-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'default' }}>
                                            ...
                                            <div style={{ textAlign: 'right' }}>
                                                <div className="fin-mono" style={{ fontSize: 13, fontWeight: 700, color: dark ? '#f1f5f9' : '#0f172a' }}>{p.value}%</div>
                                                <div style={{ fontSize: 11, color: dark ? 'rgba(148,163,184,0.50)' : 'rgba(51,65,85,0.50)' }}>{fmtK(p.amount)}</div>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                {mockData.revenueByProvider.map(p => (
                                    <div key={p.name} className="fin-provider-card" style={{
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                                            <span style={{ fontSize: 14, fontWeight: 700, color: dark ? '#f1f5f9' : '#0f172a' }}>{p.name}</span>
                                            <span className="fin-mono" style={{ fontSize: 14, fontWeight: 700, color: '#10b981', letterSpacing: '-0.01em' }}>{fmtK(p.revenue)}</span>
                                        </div>
                                        <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                                            {[`wRVU: ${p.rvu.toLocaleString()}`, `Exp: ${fmtK(p.expenses)}`].map(tag => (
                                                <span key={tag} style={{
                                                    fontSize: 11, fontWeight: 500, color: dark ? 'rgba(148,163,184,0.55)' : 'rgba(51,65,85,0.55)',
                                        ...
                                        <div style={{ height: 5, borderRadius: 999, background: dark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                                            <div style={{
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 12 }}>
                            {mockData.expenses.categories.map((c, idx) => (
                                <div key={c.name} className="fin-exp-card" style={{
                                    ...
                                    <div className="fin-mono" style={{ fontSize: 22, fontWeight: 800, color: dark ? '#f1f5f9' : '#0f172a', letterSpacing: '-0.02em', marginBottom: 4 }}>{fmtK(c.amount)}</div>
                                    <div style={{ fontSize: 13, fontWeight: 600, color: dark ? 'rgba(148,163,184,0.60)' : 'rgba(51,65,85,0.60)', marginBottom: 12 }}>{c.name}</div>
                                    <div style={{ height: 5, borderRadius: 999, background: dark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                                        <div style={{
                                            height: '100%', borderRadius: 999,
                ...
                <div key="ar" style={{ display: 'flex', flexDirection: 'column', gap: 16 }} className="fin-enter">
                    ...
                    <div className="fin-glass fin-enter rounded-2xl p-6">
                            ...
                            {mockData.arAging.map(b => (
                                <div key={b.bucket} className="fin-ar-row" style={{ padding: '10px 12px', borderRadius: 12, cursor: 'default' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                                        <span style={{ fontSize: 14, fontWeight: 700, color: dark ? '#f1f5f9' : '#0f172a' }}>{b.bucket}</span>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                                            <span style={{ fontSize: 12, fontWeight: 500, color: dark ? 'rgba(148,163,184,0.55)' : 'rgba(51,65,85,0.55)', minWidth: 32, textAlign: 'right' }}>{b.pct}%</span>
                                            <span className="fin-mono" style={{ fontSize: 14, fontWeight: 700, color: dark ? '#f1f5f9' : '#0f172a', minWidth: 70, textAlign: 'right', letterSpacing: '-0.01em' }}>{fmtK(b.amount)}</span>
                                        </div>
                                    </div>
                                    <div style={{ height: 7, borderRadius: 999, background: dark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.03)', overflow: 'hidden' }}>
                                        <div style={{
                                            height: '100%', borderRadius: 999,
                                            width: `${b.pct}%`,
                        ...
                        <div style={{ marginTop: 20 }}>
                            <AlertBanner
                                icon={<AlertTriangle size={22} color="#f59e0b" strokeWidth={1.5}/>}
                    ...
                    <div className="fin-stagger" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 14 }}>
                        {[
                            ...
                            <div key={f.label} className="fin-glass fin-proj-card fin-enter rounded-2xl p-6" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 10, cursor: 'default' }}>
                                <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: dark ? 'rgba(148,163,184,0.55)' : 'rgba(51,65,85,0.55)' }}>{f.label}</div>
                                <div className="fin-mono" style={{ fontSize: 30, fontWeight: 800, color: dark ? '#f1f5f9' : '#0f172a', letterSpacing: '-0.03em', lineHeight: 1 }}>{f.value}</div>
                                <span style={{
                                    fontSize: 13, fontWeight: 700, color: '#10b981',
                                    background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.25)',
                                    padding: '4px 12px', borderRadius: 999,
                                }}>{f.trend} vs today</span>
                                <p style={{ fontSize: 11, color: dark ? 'rgba(148,163,184,0.40)' : 'rgba(51,65,85,0.40)', lineHeight: 1.5, marginTop: 4 }}>Based on historical run-rate + contracted revenue</p>
                            </div>
                        ))}
                    </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 12 }}>
                            {[
                                ...
                                <div key={b.metric} className="fin-bench-card fin-enter" style={{
                                    ...
                                    <div style={{ fontSize: 12, fontWeight: 600, color: dark ? 'rgba(148,163,184,0.55)' : 'rgba(51,65,85,0.55)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{b.metric}</div>
                                    <div className="fin-mono" style={{ fontSize: 24, fontWeight: 800, color: dark ? '#f1f5f9' : '#0f172a', letterSpacing: '-0.02em', marginBottom: 12 }}>{b.yours}</div>
                                    <div style={{
                                        paddingTop: 12,
                                        borderTop: `1px solid ${dark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)'}`,
                                        display: 'flex', alignItems: 'center', gap: 6, fontSize: 12,
                                    }}>
                                        <span style={{ color: dark ? 'rgba(148,163,184,0.45)' : 'rgba(51,65,85,0.45)' }}>Benchmark:</span>
                                        <span style={{ fontWeight: 700, color: '#f59e0b' }}>{b.benchmark}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
